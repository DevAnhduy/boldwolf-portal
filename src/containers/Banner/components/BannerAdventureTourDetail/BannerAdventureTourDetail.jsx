...
import "./BannerAdventureTourDetail.scss";

const BannerAdventureTourDetail = ({
  updateBanner,
  addBanner,
  token,
  match,
  ...
  useEffect(() => {
    onGetBannerDetailWithReference({
      token,
      data: { key: "adventure_tour_id", id: match.params.id },
  ...
  const onImgChanged = async (value) => {
    const valueBody = {
      ...bannerData,
      banner_img: value.id,
      adventure_tour_id: match.params.id,
    };
    if (!img) {
      if (bannerData.id) {
        updateBanner({
  ...
  const submitContent = (values) => {
    ...
    if (bannerData.id) {
      ...
      addBanner({
        token,
        data: { ...values, adventure_tour_id: match.params.id },
        fCallBack: submitContentCallBack,
      });
    }
  };
  ...
  return (
    <div className="banner-tour-detail u-banner-page">
      <div className="top">
        <BackButton />
        <Heading>Adventure Tour banner</Heading>
      </div>
