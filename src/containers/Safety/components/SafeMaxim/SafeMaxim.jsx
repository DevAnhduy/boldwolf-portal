...
import ModalSafeMaxim from '../ModalSafeMaxim/ModalSafeMaxim';

const SafeMaxim = ({ safety, token, onDeleteSafeByType }) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: "id",
            align: "center",
            render: (id) => <ActionWithOpenModalInTableCommon
                idRow={id}
                onClickDetail={showDetail} onClickDelete={openModalDelete} />
        },
    ];
    ...
    const onDelete = () => {
        ...
        onDeleteSafeByType({ token, data: modalDelete.id, fCallBack: onDeleteCallBack }, SAFE_TYPE.MAXIM)
    }
    ...
    return (
        <div className="safe-facility u-page-data">
            ...
            <div className="content">
                <TableCommon
                    columns={columns}
                    dataSource={safety && filterDataByType(safety, SAFE_TYPE.MAXIM)}
                    bordered
                    pagination={false}
                />
            </div>
            {
                visible &&
                <ModalSafeMaxim visible={visible} handleCancel={handleCancelModal} idUpdate={idUpdate} />
            }
            <ModalConfirm
...
export default connect(mapStateToProps, { onDeleteSafeByType })(SafeMaxim);
